...
assert.equal(fs.existsSync(path.join(root, 'src', 'device', 'device-authenticator.ts')), false, 'legacy Desktop Commander Remote authenticator must be removed');
...
assert.doesNotMatch(deviceSource, /MCP_SERVER_URL|mcp\.desktopcommander\.app|RemoteChannel|fetchSupabaseConfig|persistSession/, 'device runtime must only use the direct MCP Device gateway transport');

console.log('MCP Device has no legacy Desktop Commander Remote transport');
